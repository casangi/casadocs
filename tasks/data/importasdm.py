...
Parameters
   ...
   - **createmms** (bool) - Create a Multi-MS output
   - **corr_mode** (string) - Specifies the correlation mode to be considered on input. A quoted string containing a sequence of ao, co, ac,or all separated by whitespaces is expected
   - **srt** (string) - Specifies the spectral resolution type to be considered on input. A quoted string containing a sequence of fr, ca, bw, or all separated by whitespaces is expected
   - **time_sampling** (string) - Specifies the time sampling (INTEGRATION and/or SUBINTEGRATION)  to be considered on input. A quoted string containing a sequence of i, si, or all separated by whitespaces is expected
   - **ocorr_mode** (string) - Output data for correlation mode AUTO_ONLY (ao) or CROSS_ONLY (co) or CROSS_AND_AUTO (ca)
   - **compression** (bool) - Flag for turning on data compression
   - **lazy** (bool) - Make the MS DATA column read the ASDM Binary data directly (faster import, smaller MS)
   ...
   - **wvr_corrected_data** (string) - Specifies which values are considerd in the SDM binary data to fill the DATA column in the MAIN table of the MS; yes for corrected, no for uncorrected, both for corrected and uncorrected (resulting in two MSs)
   ...
   - **ignore_time** (bool) - All the rows of the tables Feed, History, Pointing, Source, SysCal, CalDevice, SysPower, and Weather are processed independently of the time range of the selected exec block / scan.
   - **process_syspower** (bool) - Process the SysPower table?
   - **process_caldevice** (bool) - Process the CalDevice table?
   - **process_pointing** (bool) - Process the Pointing table?
   - **process_flags** (bool) - Create online flags in the FLAG_CMD sub-table?
   - **flagbackup** (bool) - Back up flag column before applying flags.
   - **verbose** (bool) - Output lots of information while the filler is working
   - **overwrite** (bool) - Over write an existing MS(s)
   - **showversion** (bool) - Report the version of asdm2MS being used
   - **useversion** (string) - Version of asdm2MS to be used (v3 default, should work for all data)
   - **bdfflags** (bool) - Set the MS FLAG column according to the ASDM _binary_ flags
   - **with_pointing_correction** (bool) - Add (ASDM::Pointing::encoder - ASDM::Pointing::pointingDirection) to the value to be written in MS::Pointing::direction
   - **convert_ephem2geo** (bool) - if True, convert any attached ephemerides to the GEO reference frame (time-spacing not changed)
