...
Parameters
   ...
   - **whichrep** (int) - If fits image has multiple coordinate reps, choose one.
   - **whichhdu** (int) - If fits file contains multiple images, choose one (0 = first HDU, -1 = first valid image).
   - **zeroblanks** (bool) - Set blanked pixels to zero (not NaN)
   - **overwrite** (bool) - Overwrite output file if it exists?
   - **defaultaxes** (bool) - Add the default 4D coordinate axes where they are missing; value True requires setting defaultaxesvalues
