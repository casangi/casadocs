...
Parameters
   ...
   - **fitfunc** (string='gaussian') - function for fitting ["gaussian", "lorentzian"]
   - **fitmode** (string='list') - mode for setting additional channel masks. "list" and "auto" are available now.

      .. raw:: html

         <details><summary><i> fitmode = list </i></summary>

      - **nfit** (intArray=[0]) - list of number of lines to fit in maskline region.
...
Description
   ...
   *fitmode* in **sdfit** enables fitting modes 'list' and 'auto' at
   ...
   by using MS selection syntax, while the number oflines per region
   can be specified via the *nfit* parameter.

   In 'auto' mode, the line finder detects channel ranges of spectral
   linesbased on median absolute deviation (MAD) of the spectra
   using user definedcriteria with the sub-parameters *thresh*,
