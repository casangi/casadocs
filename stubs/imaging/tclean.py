...
Parameters
   - **vis** ({string, stringArray}) - Name of input visibility file(s) [1]_
   - **selectdata** (bool=True) - Enable data selection parameters [2]_

      .. raw:: html

         <details><summary><i> selectdata = True </i></summary>

      - **field** ({string, stringArray}='') - field(s) to select [3]_
      - **spw** ({string, stringArray}='') - spw(s)/channels to select [4]_
      - **timerange** ({string, stringArray}='') - Range of time to select from data [5]_
      - **uvrange** ({string, stringArray}='') - Select data within uvrange [6]_
      - **antenna** ({string, stringArray}='') - Select data based on antenna/baseline [7]_
      - **scan** ({string, stringArray}='') - Scan number range [8]_
      - **observation** ({string, int}='') - Observation ID range [9]_
      - **intent** ({string, stringArray}='') - Scan Intent(s) [10]_

      .. raw:: html

         </details>
   - **datacolumn** (string='corrected') - Data column to image(data,corrected) [11]_
   - **imagename** ({int, string, stringArray}='') - Pre-name of output images [12]_
   - **imsize** ({int, intArray}=100) - Number of pixels [13]_
   - **cell** ({int, double, intArray, doubleArray, string, stringArray}='"1arcsec"') - Cell size [14]_
   - **phasecenter** ({int, string}='') - Phase center of the image [15]_
   - **stokes** (string='I') - Stokes Planes to make [16]_
   - **projection** (string='SIN') - Coordinate projection  [17]_
   - **startmodel** (string='') - Name of starting model image [18]_
   - **specmode** (string='mfs') - Spectral definition mode (mfs,cube,cubedata, cubesource) [19]_

      .. raw:: html

         <details><summary><i> specmode = mfs </i></summary>

      - **reffreq** (string='') - Reference frequency [20]_
      ...
      - **nchan** (int=-1) - Number of channels in the output image [21]_
      - **start** (string='') - First channel (e.g. start=3,start=\'1.1GHz\',start=\'15343km/s\') [22]_
      - **width** (string='') - Channel width (e.g. width=2,width=\'0.1MHz\',width=\'10km/s\') [23]_
      - **outframe** (string='LSRK') - Spectral reference frame in which to interpret \'start\' and \'width\' [24]_
      - **veltype** (string='radio') - Velocity type (radio, z, ratio, beta, gamma, optical) [25]_
      - **restfreq** (stringArray=['']) - List of rest frequencies [26]_
      - **interpolation** (string='linear') - Spectral interpolation (nearest,linear,cubic) [27]_
      - **chanchunks** (int=1) - Number of channel chunks [32]_
      - **perchanweightdensity** (bool=True) - whether to calculate weight density per channel in Briggs style weighting or not [28]_
   ...
   - **gridder** (string='standard') - Gridding options (standard, wproject, widefield, mosaic, awproject) [29]_

      .. raw:: html

         <details><summary><i> gridder = standard </i></summary>

      - **vptable** (string='') - Name of Voltage Pattern table [34]_
      - **pblimit** (double=0.2) - PB gain level at which to cut off normalizations  [45]_
      ...
      - **wprojplanes** (int=1) - Number of distinct w-values for convolution functions [33]_
      - **facets** (int=1) - Number of facets on a side [30]_
      - **vptable** (string='') - Name of Voltage Pattern table [34]_
      - **pblimit** (double=0.2) - PB gain level at which to cut off normalizations  [45]_
      ...
      - **normtype** (string='flatnoise') - Normalization type (flatnoise, flatsky,pbsquare) [46]_
      - **vptable** (string='') - Name of Voltage Pattern table [34]_
      - **usepointing** (bool=False) - The parameter makes the gridder utilize the pointing table phase directions while computing the residual image. [41]_
      - **mosweight** (bool=True) - Indepently weight each field in a mosaic [35]_
      - **pblimit** (double=0.2) - PB gain level at which to cut off normalizations  [45]_
      - **conjbeams** (bool=False) - Use conjugate frequency for wideband A-terms [39]_
      - **psfphasecenter** ({int, string}='') - optional direction to calculate psf for mosaic (default is image phasecenter) [31]_
      ...
      - **wprojplanes** (int=1) - Number of distinct w-values for convolution functions [33]_
      - **normtype** (string='flatnoise') - Normalization type (flatnoise, flatsky,pbsquare) [46]_
      - **psterm** (bool=False) - Use prolate spheroidal during gridding [37]_
      - **aterm** (bool=True) - Use aperture illumination functions during gridding [36]_
      - **cfcache** (string='') - Convolution function cache directory name [40]_
      - **computepastep** (double=360.0) - Parallactic angle interval after the AIFs are recomputed (deg) [42]_
      - **rotatepastep** (double=360.0) - Parallactic angle interval after which the nearest AIF is rotated (deg)  [43]_
      - **pointingoffsetsigdev** ({intArray, doubleArray}=['']) -  Pointing offset threshold to determine heterogeneity of pointing corrections for the AWProject gridder [44]_
      - **wbawp** (bool=True) - Use wideband A-terms [38]_
      - **mosweight** (bool=True) - Indepently weight each field in a mosaic [35]_
      - **pblimit** (double=0.2) - PB gain level at which to cut off normalizations  [45]_
      - **conjbeams** (bool=False) - Use conjugate frequency for wideband A-terms [39]_
      - **usepointing** (bool=False) - The parameter makes the gridder utilize the pointing table phase directions while computing the residual image. [41]_
   ...
   - **deconvolver** (string='hogbom') - Minor cycle algorithm (hogbom,clark,multiscale,mtmfs,mem,clarkstokes) [47]_

      .. raw:: html

         <details><summary><i> deconvolver = multiscale </i></summary>

      - **scales** ({intArray, doubleArray}=['']) - List of scale sizes (in pixels) for multi-scale algorithms [48]_
      - **smallscalebias** (double=0.0) - Biases the scale selection when using multi-scale or mtmfs deconvolvers  [50]_
      ...
      - **scales** ({intArray, doubleArray}=['']) - List of scale sizes (in pixels) for multi-scale algorithms [48]_
      - **nterms** (int=2) - Number of Taylor coefficients in the spectral model [49]_
      - **smallscalebias** (double=0.0) - Biases the scale selection when using multi-scale or mtmfs deconvolvers  [50]_

      .. raw:: html

         </details>
   - **restoration** (bool=True) - Do restoration steps (or not) [51]_

      .. raw:: html

         <details><summary><i> restoration = True </i></summary>

      - **restoringbeam** ({string, stringArray}='') - Restoring beam shape to use. Default is the PSF main lobe [52]_
      - **pbcor** (bool=False) - Apply PB correction on the output restored image [53]_

      .. raw:: html

         </details>
   - **outlierfile** (string='') - Name of outlier-field image definitions [54]_
   - **weighting** (string='natural') - Weighting scheme (natural,uniform,briggs, briggsabs[experimental]) [55]_

      .. raw:: html

         <details><summary><i> weighting = natural </i></summary>

      - **uvtaper** (stringArray=['']) - uv-taper on outer baselines in uv-plane [59]_
      ...
      - **robust** (double=0.5) - Robustness parameter [56]_
      - **npixels** (int=0) - Number of pixels to determine uv-cell size  [58]_
      - **uvtaper** (stringArray=['']) - uv-taper on outer baselines in uv-plane [59]_
      ...
      - **robust** (double=0.5) - Robustness parameter [56]_
      - **noise** (variant='1.0Jy') - noise parameter for briggs abs mode weighting [57]_
      - **npixels** (int=0) - Number of pixels to determine uv-cell size  [58]_
      - **uvtaper** (stringArray=['']) - uv-taper on outer baselines in uv-plane [59]_
   ...
   - **niter** (int=0) - Maximum number of iterations [60]_
   - **usemask** (string='user') - Type of mask(s) for deconvolution:  user, pb, or auto-multithresh [69]_

      .. raw:: html

         <details><summary><i> usemask = user </i></summary>

      - **mask** ({string, stringArray}='') - Mask (a list of image name(s) or region file(s) or region string(s) ) [70]_
      - **pbmask** (double=0.0) - primary beam mask [71]_
      ...
      - **pbmask** (double=0.0) - primary beam mask [71]_
      - **sidelobethreshold** (double=3.0) - sidelobethreshold *  the max sidelobe level * peak residual [72]_
      - **noisethreshold** (double=5.0) - noisethreshold * rms in residual image + location(median)  [73]_
      - **lownoisethreshold** (double=1.5) - lownoisethreshold * rms in residual image + location(median)  [74]_
      - **negativethreshold** (double=0.0) - negativethreshold * rms in residual image + location(median)  [75]_
      - **smoothfactor** (double=1.0) - smoothing factor in a unit of the beam [76]_
      - **minbeamfrac** (double=0.3) - minimum beam fraction for pruning [77]_
      - **cutthreshold** (double=0.01) - threshold to cut the smoothed mask to create a final mask [78]_
      - **growiterations** (int=75) - number of binary dilation iterations for growing the mask [79]_
      - **dogrowprune** (bool=True) - Do pruning on the grow mask [80]_
      - **minpercentchange** (double=-1.0) - minimum percentage change in mask size (per channel plane) to trigger updating of mask by automask  [81]_
      - **verbose** (bool=False) - True: print more automasking information in the logger [82]_

      .. raw:: html

         </details>
   - **fastnoise** (bool=True) - True: use the faster (old) noise calculation. False: use the new improved noise calculations [83]_
   - **restart** (bool=True) - True : Re-use existing images. False : Increment imagename [84]_
   - **savemodel** (string='none') - Options to save model visibilities (none, virtual, modelcolumn) [85]_
   - **calcres** (bool=True) - Calculate initial residual image [86]_
   - **calcpsf** (bool=True) - Calculate PSF [87]_
   - **parallel** (bool=False) - Run major cycles in parallel [88]_
