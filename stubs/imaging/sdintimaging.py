...
Parameters
   - **usedata** (string='sdint') - Output image type(int, sd, sdint) [1]_

      .. raw:: html

         <details><summary><i> usedata = sd </i></summary>

      - **sdimage** (string='') - Input single dish image [2]_
      - **sdpsf** (string='') - Input single dish PSF image [3]_
      - **sdgain** (double=1.0) - A factor or gain to adjust single dish flux scale [4]_
      - **dishdia** (double=100.0) - Effective dish diameter [5]_
   ...
   - **vis** ({string, stringArray}) - Name of input visibility file(s) [6]_
   - **selectdata** (bool=True) - Enable data selection parameters [7]_

      .. raw:: html

         <details><summary><i> selectdata = True </i></summary>

      - **field** ({string, stringArray}='') - field(s) to select [8]_
      - **spw** ({string, stringArray}='') - spw(s)/channels to select [9]_
      - **timerange** ({string, stringArray}='') - Range of time to select from data [10]_
      - **uvrange** ({string, stringArray}='') - Select data within uvrange [11]_
      - **antenna** ({string, stringArray}='') - Select data based on antenna/baseline [12]_
      - **scan** ({string, stringArray}='') - Scan number range [13]_
      - **observation** ({string, int}='') - Observation ID range [14]_
      - **intent** ({string, stringArray}='') - Scan Intent(s) [15]_

      .. raw:: html

         </details>
   - **datacolumn** (string='corrected') - Data column to image(data,corrected) [16]_
   - **imagename** ({int, string, stringArray}='') - Pre-name of output images [17]_
   - **imsize** ({int, intArray}=100) - Number of pixels [18]_
   - **cell** ({int, double, intArray, doubleArray, string, stringArray}='"1arcsec"') - Cell size [19]_
   - **phasecenter** ({int, string}='') - Phase center of the image [20]_
   - **stokes** (string='I') - Stokes Planes to make [21]_
   - **projection** (string='SIN') - Coordinate projection  [22]_
   - **startmodel** (string='') - Name of starting model image [23]_
   - **specmode** (string='mfs') - Spectral definition mode (mfs,cube,cubedata, cubesource) [24]_

      .. raw:: html

         <details><summary><i> specmode = mfs </i></summary>

      - **reffreq** (string='') - Reference frequency [25]_

      .. raw:: html

         </details>
   - **nchan** (int=-1) - Number of channels in the output image [26]_
   - **start** (string='') - First channel (e.g. start=3,start=\'1.1GHz\',start=\'15343km/s\') [27]_
   - **width** (string='') - Channel width (e.g. width=2,width=\'0.1MHz\',width=\'10km/s\') [28]_
   - **outframe** (string='LSRK') - Spectral reference frame in which to interpret \'start\' and \'width\' [29]_
   - **veltype** (string='radio') - Velocity type (radio, z, ratio, beta, gamma, optical) [30]_
   - **restfreq** (stringArray=['']) - List of rest frequencies [31]_
   - **interpolation** (string='linear') - Spectral interpolation (nearest,linear,cubic) [32]_
   - **chanchunks** (int=1) - Number of channel chunks [33]_
   - **perchanweightdensity** (bool=True) - whether to calculate weight density per channel in Briggs style weighting or not [34]_
   - **gridder** (string='standard') - Gridding options (standard, wproject, widefield, mosaic, awproject) [35]_

      .. raw:: html

         <details><summary><i> gridder = standard </i></summary>

      - **vptable** (string='') - Name of Voltage Pattern table [39]_
      - **pblimit** (double=0.2) - PB gain level at which to cut off normalizations  [49]_
      ...
      - **wprojplanes** (int=1) - Number of distinct w-values for convolution functions [38]_
      - **facets** (int=1) - Number of facets on a side [36]_
      - **vptable** (string='') - Name of Voltage Pattern table [39]_
      - **pblimit** (double=0.2) - PB gain level at which to cut off normalizations  [49]_
      ...
      - **vptable** (string='') - Name of Voltage Pattern table [39]_
      - **usepointing** (bool=False) - The parameter makes the gridder utilize the pointing table phase directions while computing the residual image. [45]_
      - **mosweight** (bool=True) - Indepently weight each field in a mosaic [40]_
      - **pblimit** (double=0.2) - PB gain level at which to cut off normalizations  [49]_
      - **psfphasecenter** ({int, string}='') - optional direction to calculate psf for mosaic (default is image phasecenter) [37]_
      ...
      - **wprojplanes** (int=1) - Number of distinct w-values for convolution functions [38]_
      - **psterm** (bool=False) - Use prolate spheroidal during gridding [42]_
      - **aterm** (bool=True) - Use aperture illumination functions during gridding [41]_
      - **cfcache** (string='') - Convolution function cache directory name [44]_
      - **computepastep** (double=360.0) - Parallactic angle interval after the AIFs are recomputed (deg) [46]_
      - **rotatepastep** (double=360.0) - Parallactic angle interval after which the nearest AIF is rotated (deg)  [47]_
      - **pointingoffsetsigdev** ({intArray, doubleArray}=['']) -  Pointing offset threshold to determine heterogeneity of pointing corrections for the AWProject gridder [48]_
      - **wbawp** (bool=True) - Use wideband A-terms [43]_
      - **mosweight** (bool=True) - Indepently weight each field in a mosaic [40]_
      - **pblimit** (double=0.2) - PB gain level at which to cut off normalizations  [49]_
      - **usepointing** (bool=False) - The parameter makes the gridder utilize the pointing table phase directions while computing the residual image. [45]_
   ...
   - **deconvolver** (string='hogbom') - Minor cycle algorithm (hogbom,clark,multiscale,mtmfs,mem,clarkstokes) [50]_

      .. raw:: html

         <details><summary><i> deconvolver = multiscale </i></summary>

      - **scales** ({intArray, floatArray}=['']) - List of scale sizes (in pixels) for multi-scale algorithms [51]_
      - **smallscalebias** (double=0.0) - Biases the scale selection when using multi-scale or mtmfs deconvolvers  [53]_
      ...
      - **scales** ({intArray, floatArray}=['']) - List of scale sizes (in pixels) for multi-scale algorithms [51]_
      - **nterms** (int=2) - Number of Taylor coefficients in the spectral model [52]_
      - **smallscalebias** (double=0.0) - Biases the scale selection when using multi-scale or mtmfs deconvolvers  [53]_

      .. raw:: html

         </details>
   - **restoration** (bool=True) - Do restoration steps (or not) [54]_

      .. raw:: html

         <details><summary><i> restoration = True </i></summary>

      - **restoringbeam** ({string, stringArray}='') - Restoring beam shape to use. Default is the PSF main lobe [55]_
      - **pbcor** (bool=False) - Apply PB correction on the output restored image [56]_

      .. raw:: html

         </details>
   - **weighting** (string='natural') - Weighting scheme (natural,uniform,briggs, briggsabs[experimental]) [57]_

      .. raw:: html

         <details><summary><i> weighting = natural </i></summary>

      - **uvtaper** (stringArray=['']) - uv-taper on outer baselines in uv-plane [61]_
      ...
      - **robust** (double=0.5) - Robustness parameter [58]_
      - **npixels** (int=0) - Number of pixels to determine uv-cell size  [60]_
      - **uvtaper** (stringArray=['']) - uv-taper on outer baselines in uv-plane [61]_
   ...
   - **niter** (int=0) - Maximum number of iterations [62]_
   - **usemask** (string='user') - Type of mask(s) for deconvolution:  user, pb, or auto-multithresh [71]_

      .. raw:: html

         <details><summary><i> usemask = user </i></summary>

      - **mask** ({string, stringArray}='') - Mask (a list of image name(s) or region file(s) or region string(s) ) [72]_
      - **pbmask** (double=0.0) - primary beam mask [73]_
      ...
      - **pbmask** (double=0.0) - primary beam mask [73]_
      - **sidelobethreshold** (double=3.0) - sidelobethreshold *  the max sidelobe level * peak residual [74]_
      - **noisethreshold** (double=5.0) - noisethreshold * rms in residual image + location(median)  [75]_
      - **lownoisethreshold** (double=1.5) - lownoisethreshold * rms in residual image + location(median)  [76]_
      - **negativethreshold** (double=0.0) - negativethreshold * rms in residual image + location(median)  [77]_
      - **smoothfactor** (double=1.0) - smoothing factor in a unit of the beam [78]_
      - **minbeamfrac** (double=0.3) - minimum beam fraction for pruning [79]_
      - **cutthreshold** (double=0.01) - threshold to cut the smoothed mask to create a final mask [80]_
      - **growiterations** (int=75) - number of binary dilation iterations for growing the mask [81]_
      - **dogrowprune** (bool=True) - Do pruning on the grow mask [82]_
      - **minpercentchange** (double=-1.0) - minimum percentage change in mask size (per channel plane) to trigger updating of mask by automask  [83]_
      - **verbose** (bool=False) - True: print more automasking information in the logger [84]_

      .. raw:: html

         </details>
   - **fastnoise** (bool=True) - True: use the faster (old) noise calculation. False: use the new improved noise calculations [85]_
   - **restart** (bool=True) - True : Re-use existing images. False : Increment imagename [86]_
   - **savemodel** (string='none') - Options to save model visibilities (none, virtual, modelcolumn) [87]_
   - **calcres** (bool=True) - Calculate initial residual image [88]_
   - **calcpsf** (bool=True) - Calculate PSF [89]_
   - **parallel** (bool=False) - Run major cycles in parallel [90]_
