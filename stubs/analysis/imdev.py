...
Parameters
   - imagename_ (string) - Input image name
   - outfile_ (string='') - Output image file name. If left blank (the default), no image is written but a new image tool referencing the collapsed image is returned.
   - region_ ({string, record}='') - Region selection. Default is to use the full image.
   ...
   - xlength_ ({string, int}='1pix') - Either x coordinate length of box, or diameter of circle. Circle is used if ylength is empty string.
   - ylength_ ({string, int}='1pix') - y coordinate length of box. Use a circle if ylength is empty string.
   - interp_ (string='cubic') - Interpolation algorithm to use. One of "nearest", "linear", "cubic", or "lanczos". Minimum match supported.
   - stattype_ (string='sigma') - Statistic to compute. See full description for supported statistics.
   - statalg_ (string='classic') - Statistics computation algorithm to use. Supported values are "chauvenet" and "classic", Minimum match is supported.
      ...
      - maxiter_ (int=-1) - For chauvenet, this is the maximum number of iterations to attempt. Iterating will stop when either this limit is reached, or the zscore criterion is met. If negative, iterate until the zscore criterion is met. Ignored if algortihm is not "chauvenet".
