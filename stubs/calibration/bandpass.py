...
Parameters
   ...
   - **caltable** (string='') - Name of output bandpass calibration table [2]_
   ...
   - **solint** (variant='inf') - Solution interval in time[,freq] [13]_
   - **combine** (string='scan') - Data axes which to combine for solve (obs, scan, spw, and/or field) [14]_
   - **refant** (string='') - Reference antenna name(s) [15]_
   - **minblperant** (int=4) - Minimum baselines _per antenna_ required for solve [16]_
   - **minsnr** (double=3.0) - Reject solutions below this SNR (only applies for bandtype = B) [17]_
   - **solnorm** (bool=False) - Normalize average solution amplitudes to 1.0  [18]_
   - **bandtype** (string='B') - Type of bandpass solution (B or BPOLY) [19]_

      .. raw:: html

         <details><summary><i> bandtype = B </i></summary>

      - **fillgaps** (int=0) - Fill flagged solution channels by interpolation [23]_
      ...
      - **degamp** (int=3) - Polynomial degree for BPOLY amplitude solution [24]_
      - **degphase** (int=3) - Polynomial degree for BPOLY phase solution [25]_
      - **visnorm** (bool=False) - Normalize data prior to BPOLY solution [26]_
      - **maskcenter** (int=0) - Number of channels to avoid in center of each band [27]_
      - **maskedge** (int=5) - Fraction of channels to avoid at each band edge (in %) [28]_

      .. raw:: html

         </details>
   - **smodel** (doubleArray=['']) - Point source Stokes parameters for source model. [20]_
   - **corrdepflags** (bool=False) - Respect correlation-dependent flags [21]_
   - **append** (bool=False) - Append solutions to the (existing) table [22]_
   - **docallib** (bool=False) - Use callib or traditional cal apply parameters [29]_

      .. raw:: html

         <details><summary><i> docallib = False </i></summary>

      - **gaintable** (stringArray=['']) - Gain calibration table(s) to apply on the fly [31]_
      - **gainfield** (stringArray=['']) - Select a subset of calibrators from gaintable(s) [32]_
      - **interp** (stringArray=['']) - Interpolation parameters for each gaintable, as a list [33]_
      - **spwmap** (intArray=['']) - Spectral window mappings to form for gaintable(s) [34]_
      ...
      - **callib** (string='') - Cal Library filename [30]_

      .. raw:: html

         </details>
   - **parang** (bool=False) - Apply parallactic angle correction [35]_
