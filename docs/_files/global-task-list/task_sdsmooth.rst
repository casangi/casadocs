...
.. container:: section
   ...
   .. container::
      ...
      *spw*, *scan*, *timerange*, *pol,* and *intent*.
      ...
      Wkout=(∑ni=1ci)2(∑ni=1c2i/Wj+iinp)\ :math:`W^{k}_{\rm out} = \frac{ (\sum_{i=1}^{n} c_{i})^{2} } { (\sum_{i=1}^{n} c^{2}_{i} / W^{j+i}_{\rm inp}) } `
         ,

      where W\ :math:`W` is the input/output weight, c\ :math:`c` is the
      spectral smoothing kernel originated from a channel
      k\ :math:`k` where the weight is evaluated, and n\ :math:`n` is
      the width of the spectral smoothing kernel in channels. In the
      case of Gaussian, n\ :math:`n` corresponds to FWHM in channels.
