...
.. container:: section
   ...
   .. container::
      ...
      c\ 3\ \*ln(x/div)2`\ :math:`^2` + ... +
      c\ :sub:`n`\ \*ln(x/div)(n−1)\ :math:`^{(n-1)}`]

      In the case of fitting a spectral index, which is traditionally
      represented as α\ :math:`\alpha`, the α\ :math:`\alpha` is equal
      to c\ :sub:`1`.
      ...
      div =
      10\ :sup:`int[log10√min(x)∗max(x)]`\ :math:`int[log_{10}\sqrt{min(x)*max(x)}]`
      ...
      estimates at every pixel. So, for example, if S(ν\ :math:`\nu`) is
      proportional to να\ :math:`\nu^{\alpha}` and you expect alpha to
      be near -0.8 and the value of S(ν\ :math:`\nu`) is 1.5 at
      109\ :math:`^9` Hz and your image(s) have spectral units of Hz,
      you would specify *spxest=[1.5, -0.8]* and *div=1e9* when fitting
      a plp function, or *spxest=[0.405, -0.8]* and *div=1e9* if fitting
      an ltp function close to ln(1.5) - 0.8*ln(x/div).
      ...
      *sigma* parameter. This parameter takes either an array or an
      ...
      weight = 1σ2\ :math:`\frac{1}{\sigma^2}`
