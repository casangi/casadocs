...
# loop through each task xml file and parse the xml to python dictionaries
tasklist = []
for task in os.listdir('xml'):
    if not task.endswith('.xml'): continue
    #print('processing ' + task)
    
    with open('xml/'+task, 'r') as fid:
        xmlstring = fid.read()
...
for task in tasklist:
    ...
    rst = ''
    if os.path.exists('markdown/global-task-list/task_' + task['name'] + '.rst'):
        with open('markdown/global-task-list/task_' + task['name'] + '.rst', 'r') as fid:
            rst = fid.read()
