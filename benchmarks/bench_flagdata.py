...
class BaseFlagSetup():
    ...
    datapath = ctsys.resolve("unittest/testdata/")
...
class ListMode(BaseFlagSetup):
    ...
    def setup_cache(self):
        self.flags_cmd = "uid___A002_Xe1f219_X6d0b.flagcmds.txt"
        if not os.path.exists(self.flags_cmd):
            shutil.copyfile(os.path.join(self.datapath, self.flags_cmd))
...
class ShadowMode(BaseFlagSetup):
    ...
    def time_shadow_absent_antennas(self):
        ...
        flagdata(vis=self.vis, mode='shadow', tolerance=0.0, addantenna=self.inpfile1, flagbackup=False)
